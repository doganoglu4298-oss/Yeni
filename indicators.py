...
def calculate_vwap(
    ...
    """
    Calculate a rolling VWAP over the given period (in candles).

    Using a fixed rolling window (instead of a cumulative sum over the
    whole history) keeps VWAP responsive to recent price/volume action,
    which is what the "above/below VWAP" filters in strategy.py expect.
    """
...
def calculate_supertrend(
    ...
    # ATR is NaN during its warm-up period, which makes basic_upper /
    # basic_lower NaN too. The recursive "carry forward previous value"
    # rule below relies on comparisons like `x < final_upper[i-1]` — and
    # ANY comparison against NaN evaluates to False in Python. So if we
    # started the recursion at row 0 (while ATR is still NaN), a NaN
    # would get locked into final_upper/final_lower and propagate
    # forward forever, since the flip condition could never become
    # True again. Downstream this silently forced supertrend_direction
    # to -1 (bearish) for the entire series, no matter what the actual
    # trend was. Fix: start the recursion at the first row where the
    # bands are actually valid numbers.
    valid_mask = basic_upper.notna() & basic_lower.notna()
    ...
    start = int(valid_mask.to_numpy().argmax())
    ...
    if start > 0:
        supertrend.iloc[:start] = float("nan")
        direction.iloc[:start] = 0
