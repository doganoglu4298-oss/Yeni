    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                # Dinamik Grid Aralığı Güncelleme (saatlik)
                if self.config.grid.dynamic_grid:
                    # Basit saatlik kontrol (her 12 döngüde bir ≈ 1-2 dakika)
                    if not hasattr(self, 'last_recenter_time'):
                        self.last_recenter_time = time.time()

                    if time.time() - self.last_recenter_time > (self.config.grid.recenter_hours * 3600):
                        self.strategy.calculate_dynamic_grid_range(df, self.config.grid.dynamic_range_lookback_hours)
                        self.strategy.calculate_grid_levels(current_price)  # Yeni aralığa göre seviyeleri güncelle
                        self.last_recenter_time = time.time()
                        logger.info("Grid aralığı saatlik olarak güncellendi.")
