...
class SymbolConfig:
    ...
    base_asset: str = "BTC"
    leverage: int = 8                       # Önerilen: 5-10x (Grid için yüksek bırakmayın!)
...
class GridConfig:
    ...
    grid_count: int = 28                    # Grid seviyesi sayısı (20-40 arası ideal)
    grid_step_type: Literal["arithmetic", "geometric"] = "arithmetic"
    
    # Her grid seviyesinde açılacak pozisyon büyüklüğü
    order_size_usdt: float = 25.0           # USDT cinsinden (sermayene göre ayarla)
    # Alternatif: sermayenin %'si olarak
    order_size_pct_of_balance: float = 0.0  # 0 ise order_size_usdt kullanılır
    
    # === Gelişmiş Grid Yönetimi ===
    dynamic_grid: bool = True               # Fiyat aralığını periyodik güncellesin mi?
    recenter_hours: int = 24                # Kaç saatte bir grid'i yeniden ortalasın
    
    take_profit_per_grid: float = 0.8       # % (her başarılı grid işlemi sonrası)
    # Not: Vadeli grid'de genellikle her emir TP ile kapatılır
    
    # === Risk Limitleri (ÇOK ÖNEMLİ!) ===
    max_total_exposure_usdt: float = 800.0  # Toplam açık pozisyon limiti (USDT)
    bot_stop_loss_pct: float = 9.0          # Grid range dışına çıkarsa bot % kaç zararla dursun?
    max_daily_loss_usdt: float = 150.0      # Günlük max zarar limiti
    
    # Trend yakalanırsa pozisyon birikmesin diye
    max_open_grids: int = 18                # Aynı anda max açık grid seviyesi
...
class RiskManagerConfig:
    ...
    max_daily_loss_usdt: float = 150.0      # Günlük max zarar limiti (USDT)
