...
class GridStrategy:
    ...
    def calculate_dynamic_grid_range(self, df: pd.DataFrame, lookback_hours: int = 1):
        """
        Son X saatlik mumlara bakarak grid aralığını otomatik belirler.
        Kullanıcı "saatlik mumda en yüksek ve en düşük fiyat" istediği için bunu uygular.
        """
        if len(df) < 12:  # En az 1 saat veri olmalı (5m timeframe için ~12 mum)
            logger.warning("Yeterli veri yok, dinamik aralık hesaplanamadı.")
            return

        # Son lookback_hours saatlik veriyi al
        lookback_candles = lookback_hours * 12  # 5m timeframe varsayımı
        recent = df.tail(min(lookback_candles, len(df)))

        new_lower = recent['low'].min()
        new_upper = recent['high'].max()

        # Çok dar aralık olmasın diye minimum %1.5 genişlik kontrolü
        min_width = self.current_price * 0.015
        if (new_upper - new_lower) < min_width:
            new_upper = self.current_price + min_width / 2
            new_lower = self.current_price - min_width / 2

        self.lower_price = round(new_lower, 2)
        self.upper_price = round(new_upper, 2)

        logger.info(f"Dinamik grid aralığı güncellendi → {self.lower_price} - {self.upper_price}")
